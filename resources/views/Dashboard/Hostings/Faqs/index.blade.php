                    ...
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="page-title-right d-flex justify-content-end">
                                <div class="btn btn-list">
                                    <a href="{{ route('dashboard.hostings.faqs.create',$hosting->id) }}"><button class="btn ripple btn-primary"><i class="fas fa-plus-circle"></i> {{__('dashboard.add')}}</button></a>
                                </div>
                        ...
                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                            ...
                            <tbody>
                                ...
                                <tr id="{{ $faq->id }}">
                                    ...
                                    <td><a href="{{ route('dashboard.hostings.faqs.edit', [$hosting->id,$faq->id]) }}">{{ $faq->id}}</a></td>
                                    <td><a href="{{ route('dashboard.hostings.faqs.edit', [$hosting->id,$faq->id]) }}">{{$faq->question }}</a></td>
                                   
                                    <td>
                                        <a href="{{ route('dashboard.hostings.faqs.edit', [$hosting->id,$faq->id]) }}" class="status">
                                            @if($faq->status == 1) {{ __('dashboard.yes') }} @else {{
                                            __('dashboard.no') }} @endif
                                        </a>
                                    </td>
                                    <td>
                                        <form action="{{ route('dashboard.hostings.faqs.destroy',[$hosting->id,$faq->id]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
