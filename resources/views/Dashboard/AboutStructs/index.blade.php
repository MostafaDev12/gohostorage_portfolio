                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <div class="page-title-right d-flex justify-content-end">
                            <x-dashboard.partials.action-buttons createUrl="{{ route('dashboard.about-structs.create') }}" />
                        </div>
                    </div>
                    <div class="card-body">
                        <table id="datatable-buttons" class="table table-bordered dt-responsive nowrap w-100">
                            ...
                            <tbody>
                                ...
                                <tr id="{{ $about_struct->id }}">
                                    ...
                                    <td><a href="{{ route('dashboard.about-structs.edit', $about_struct->id) }}">{{ $about_struct->id}}</a></td>
                                    <td><a href="{{ route('dashboard.about-structs.edit', $about_struct->id) }}">{{$about_struct->title_en }}</a></td>
                                    <td><a href="{{ route('dashboard.about-structs.edit', $about_struct->id) }}">{{
                                            $about_struct->title_ar }}</a></td>
                                    <td>
                                        <a href="{{ route('dashboard.about-structs.edit', $about_struct->id) }}" class="status">
                                            @if($about_struct->status == 1) {{ __('dashboard.yes') }} @else {{
